...
# GroupKFold-honest blend weights (lgb/cat/nn) + few trees (LGB best_iter~200 on unseen time_ids)
W_LGB, W_CAT, W_NN = 0.74, 0.16, 0.10
N_LGB, N_CAT = 250, 1500
NN_SEEDS = (0, 1, 2)        # seed-averaged -> robust to the rare NN divergence seen in CV
NN_EPOCHS = 55
...
# Kaggle's default GPU is a P100 (sm_60) which this torch build can't run -> NN on CPU.
DEVICE_NN = "cpu"


def _log_scale_fit(X):
    logcols = (np.nanmin(X, 0) >= 0) & (np.nanmax(X, 0) > 5 * (np.nanmean(X, 0) + 1e-9))
    Xl = X.copy(); Xl[:, logcols] = np.log1p(np.clip(Xl[:, logcols], 0, None))
    return logcols, np.nanmean(Xl, 0), np.nanstd(Xl, 0) + 1e-6


def _log_scale(X, logcols, mu, sd):
    Xl = X.copy(); Xl[:, logcols] = np.log1p(np.clip(Xl[:, logcols], 0, None))
    return np.clip((Xl - mu) / sd, -8, 8).astype(np.float32)


def train_nn_full(Xnum, stock_idx, y, n_stocks, Xtest, stock_test, seed):
    torch.manual_seed(seed); np.random.seed(seed)
    logcols, mu, sd = _log_scale_fit(Xnum)          # log1p heavy-tailed cols -> NN-friendly
    Xn = _log_scale(Xnum, logcols, mu, sd)
    Xt = _log_scale(Xtest, logcols, mu, sd)
    ...
    model = TabularMLP(Xnum.shape[1], n_stocks).to(DEVICE_NN)
    with torch.no_grad():
    ...
    for ep in range(NN_EPOCHS):
        model.train()
        for xb, sb, yb in dl:
            xb, sb, yb = xb.to(DEVICE_NN), sb.to(DEVICE_NN), yb.to(DEVICE_NN)
            opt.zero_grad(); rmspe_loss(model(xb, sb), yb).backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            opt.step()
        sched.step()
    model.eval()
    with torch.no_grad():
        return model(torch.tensor(Xt).to(DEVICE_NN),
                     torch.tensor(stock_test).to(DEVICE_NN)).cpu().numpy()
