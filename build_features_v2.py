"""Build V2 feature tables (per-bucket + cross-sectional + KNN) for train and test.

  python build_features_v2.py            # all stocks
  python build_features_v2.py --limit 4  # smoke test
"""
...
def main(limit):
    ...
    suffix = f"_v2{'_lim'+str(limit) if limit else ''}"
    ...
    if test_csv_path.exists() and not limit:
        ...
        te.to_parquet(OUT / "test_features_v2.parquet")
        print(f"test_features_v2: {te.shape}")
