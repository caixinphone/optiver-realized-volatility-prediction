"""Train LGB + CatBoost on V2 features, optimise RMSPE, report honest CV, blend.

  python train_v2.py                      # full V2 features
  python train_v2.py --feats train_features_v2_lim4.parquet --no-test   # smoke
  python train_v2.py --cv group           # GroupKFold by time_id (honest generalisation)
...
def main(args):
    ...
    if args.no_test or not (do_lgb and do_cat):
        return
    test_path = OUT / "test_features_v2.parquet"
    if not test_path.exists():
        print("No test_features_v2.parquet -- skip submission."); return
    ...
    sub.to_csv(OUT / "submission_v2.csv", index=False)
    print(f"Wrote submission_v2.csv ({len(sub):,})  LGB n_est={n_lgb} CAT n_est={n_cat}")
